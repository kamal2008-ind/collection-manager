<div x-data x-show="$wire.shareDrawerOpen" x-cloak class="fixed inset-0 z-[9999]">
    ...
    <div class="absolute right-0 top-0 flex h-full w-full max-w-xl flex-col bg-white shadow-xl">
        <div class="flex items-center justify-between border-b px-6 py-5">
            <div>
                <h2 class="text-2xl font-bold">Share Movie</h2>

                @if ($sharingMovie)
                    <p class="mt-1 text-sm text-gray-500">{{ $sharingMovie->title }}</p>
                @endif
            </div>
            ...
            <div>
                ...
                @else
                    <p class="rounded-lg border border-dashed p-4 text-gray-500">
                        This private movie is not shared with anyone yet.
                    </p>
                @endif
            </div>

            <div class="rounded-lg bg-gray-50 p-4 text-sm text-gray-500">
                Shared users can only view this movie. They cannot edit, trash, attach, detach, share, or change
                settings.
            </div>
        </div>
