    ...
    <div class="absolute right-0 top-0 flex h-full w-full max-w-xl flex-col bg-white shadow-xl">
        ...
        <div class="flex-1 space-y-6 overflow-y-auto px-6 py-6">
            ...
            <div>
                ...
                <input
                    type="text"
                    wire:model.live.debounce.400ms="shareSearch"
                    ...
                    <div class="mt-4 overflow-hidden rounded-lg border bg-white">
                        @foreach ($shareSearchResults as $user)
                            <div class="flex items-center justify-between border-b px-4 py-3 last:border-b-0">
                                ...
                                <x-ui.button
                                    variant="primary"
                                    wire:click="shareWithUser({{ $user['id'] }})"
                                >
                                    Share
                                </x-ui.button>
                            </div>
            ...
            <div>
                    ...
                    <div class="overflow-hidden rounded-lg border bg-white">
                        @foreach ($sharedUsers as $user)
                            <div class="flex items-center justify-between border-b px-4 py-3 last:border-b-0">
                                ...
                                <button
                                    type="button"
                                    wire:click="removeSharedUser({{ $user['id'] }})"
                ...
                @else
                    <p class="rounded-lg border border-dashed p-4 text-gray-500">
                        This private workspace is not shared with anyone yet.
                    </p>
                @endif
            </div>

            <div class="rounded-lg bg-gray-50 p-4 text-sm text-gray-500">
                Shared users can only view this workspace. They cannot edit, trash,
                favorite, or share it further.
            </div>
        </div>
